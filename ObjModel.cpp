...
void ObjModel::draw(glm::vec3 position)
{
...
void ObjModel::draw()
{
	...
	for (int objGroupIndex = 0; objGroupIndex < groups.size(); ++objGroupIndex)
	{

		ObjGroup* group = groups[objGroupIndex];

		//set material texture if available
		//set material color, if available

		std::list<Face> faces = group->faces;

		std::list<Face>::iterator faceIterator;

		for (faceIterator = faces.begin(); faceIterator != faces.end(); ++faceIterator)
		{
			std::list<Vertex> vertexList = faceIterator->vertices;

			std::list<Vertex>::iterator vertextIterator;

			tigl::begin(GL_TRIANGLES);

			for (vertextIterator = vertexList.begin(); vertextIterator != vertexList.end(); ++vertextIterator)
			{
				tigl::addVertex(tigl::Vertex::PCN(vertices.at(vertextIterator->position), glm::vec4(0.35f, 0.35f, 0.35f, 1), normals.at(vertextIterator->normal)));
			}
...
void ObjModel::draw(glm::vec3 position, glm::vec4 color)
{
	glm::mat4 modelMatrix(1.0f);
	...
	for (int objGroupIndex = 0; objGroupIndex < groups.size(); ++objGroupIndex)
	{
		...
		for (faceIterator = faces.begin(); faceIterator != faces.end(); ++faceIterator)
		{
			...
			for (vertextIterator = vertexList.begin(); vertextIterator != vertexList.end(); ++vertextIterator)
			{
				tigl::addVertex(tigl::Vertex::PCN(vertices.at(vertextIterator->position), color, normals.at(vertextIterator->normal)));
			}
